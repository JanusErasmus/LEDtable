...
#define HCI_READ_PACKET_NUM_MAX 		 (6)

#define MIN(a,b)            ((a) < (b) )? (a) : (b)
#define MAX(a,b)            ((a) > (b) )? (a) : (b)

cyg_mutex_t mISRmutex;
...
void HCI_Process(void)
{
	...
	/* Explicit call to HCI_Isr(), since it cannot be called by ISR if IRQ is kept high by BlueNRG. */
	HCI_Isr();
	Enable_SPI_IRQ();
}
