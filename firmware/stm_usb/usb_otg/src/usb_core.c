                                ...
                                cyg_uint16            len)
{
  ...
  {
    cyg_uint32 count32b= 0 , i= 0;
    volatile cyg_uint32 *fifo;
    
    count32b =  (len + 3) / 4;
    fifo = pdev->regs.DFIFO[ch_ep_num];
    for (i = 0; i < count32b; i++, src+=4)
    {
      USB_OTG_WRITE_REG32( fifo, *((__packed cyg_uint32 *)src) );
    }
  }
  return status;
}
...
{
  cyg_uint32 i=0;
  cyg_uint32 count32b = (len + 3) / 4;
  
  volatile cyg_uint32 *fifo = pdev->regs.DFIFO[0];
  
  for ( i = 0; i < count32b; i++, dest += 4 )
  {
    *(__packed cyg_uint32 *)dest = USB_OTG_READ_REG32(fifo);
                               ...
                               USB_OTG_CORE_ID_TypeDef coreID)
{
	...
	pdev->regs.HPRT0 = (cyg_uint32 *)(baseAddress + USB_OTG_HOST_PORT_REGS_OFFSET);
	...
	for (i = 0; i < pdev->cfg.host_channels; i++)
	{
		pdev->regs.DFIFO[i] = (cyg_uint32 *)(baseAddress + USB_OTG_DATA_FIFO_OFFSET +\
				(i * USB_OTG_DATA_FIFO_SIZE));
	}
	pdev->regs.PCGCCTL = (cyg_uint32 *)(baseAddress + USB_OTG_PCGCCTL_OFFSET);
...
* @retval USB_OTG_STS : status
*/
USB_OTG_STS USB_OTG_SetCurrentMode(USB_OTG_CORE_HANDLE *pdev , cyg_uint8 mode)
{
  USB_OTG_STS status = USB_OTG_OK;
  USB_OTG_GUSBCFG_TypeDef  usbcfg;
...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE *pdev , cyg_uint8 hc_num)
{
  ...
  USB_OTG_GINTMSK_TypeDef  intmsk;
  cyg_uint16                 len_words = 0;
  
  cyg_uint16 num_packets;
  cyg_uint16 max_hc_pkt_count;
...
USB_OTG_STS USB_OTG_EPSetStall(USB_OTG_CORE_HANDLE *pdev , USB_OTG_EP *ep)
{
  USB_OTG_STS status = USB_OTG_OK;
  USB_OTG_DEPCTL_TypeDef  depctl;
  volatile cyg_uint32 *depctl_addr;
...
void USB_OTG_EP0_OutStart(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (pdev->cfg.dma_enable == 1)
  {
    ...
    USB_OTG_WRITE_REG32( &pdev->regs.OUTEP_REGS[0]->DOEPDMA, 
                        (cyg_uint32)&pdev->dev.setup_packet);
